...
const SelState = { SINGLE: 0, MULTIPLE: 1 };
...
class UIOutliner extends UIDiv {
  constructor(editor) {
    ...
    this.state = SelState.SINGLE;

    let whePressed = false;

    this.hasFocus = false;

    function shortcutDown(event) {
      if (whePressed) return;

      switch (event.key) {
        case "Control":
          scope.state = SelState.MULTIPLE;
          break;
      }
    ...
    this.dom.onfocus = function () {
      editor.addEventListener("editorKeyDown", shortcutDown);
      editor.addEventListener("editorKeyUp", shortcutUp);
    };

    this.dom.onblur = function () {
      editor.removeEventListener("editorKeyDown", shortcutDown);
      editor.removeEventListener("editorKeyUp", shortcutUp);
    };
  ...
  selectIndex(index) {
    if (index >= 2 && index < this.options.length) {
  ...
  setOptions(options) {
    ...
    function onClick() {
      if (
        this.value == scope.editor.scene.id ||
        this.value == scope.editor.camera.id
      )
        return;

      let tempSelected = new Array();
      if (scope.state == SelState.SINGLE) {
        tempSelected[0] = this.value;
      } else if (scope.state == SelState.MULTIPLE) {
        tempSelected = scope.selectedValue.concat(this.value);
      }

      scope.setValue(tempSelected);
    ...
    function onDrop(event) {
      ...
      // const objects = scene.getObjectById(currentDrag.value);
      const objects = scope.editor.selectionHelper.selectedObject;

      const area = event.offsetY / this.clientHeight;

      if (area < 0.25) {
        const nextObject = scene.getObjectById(this.value);
        moveObject(objects, nextObject.parent, nextObject);
      } else if (area > 0.75) {
        ...
        if (this.nextSibling !== null) {
          ...
          // end of list (no next objects)

          nextObject = null;
          parent = scene.getObjectById(this.value).parent;
        }

        moveObject(objects, parent, nextObject);
      } else {
        const parentObject = scene.getObjectById(this.value);
        moveObject(objects, parentObject);
      }
    }

    function moveObject(objects, newParent, nextObject) {
      ...
      editor.execute(
        new MoveObjectCommand(editor, objects, newParent, nextObject)
      );
  ...
  setValue(selObjsID) {
    ...
    for (let i = 0; i < that.options.length; i++) {
      const element = that.options[i];

      if (selObjsID.includes(element.value)) {
        element.classList.add("active");
        that.selectedValue.push(element.value);
        that.selectedIndex = i;
        lastSelElement = element;
      } else {
    ...
    // scroll into view
    if (lastSelElement) {
      const y = lastSelElement.offsetTop - this.dom.offsetTop;
      const bottomY = y + lastSelElement.offsetHeight;
