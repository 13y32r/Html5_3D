...
class ViewHelper extends THREE.Object3D {
	...
	constructor(editorCamera, dom) {
		...
		this.update = function (delta) {
			...
			editorCamera.position.set(camera_Up.z, camera_Up.x, camera_Up.y).applyQuaternion(q1).multiplyScalar(radius).add(focusPoint);

			// animate orientation
			editorCamera.quaternion.rotateTowards(targetQuaternion, step);
			console.log(editorCamera.quaternion);

			if (editorCamera.quaternion.angleTo(targetQuaternion) === 0) {
		...
		const camUpIndex = camera_Up.toArray().indexOf(1);

		let camInitIndex = camUpIndex + 1;
		if (camInitIndex > 2) camInitIndex = 0;
		let camInitPosArray = new Array();
		for (let i = 0; i < 3; i++) {
			if (i != camInitIndex)
				camInitPosArray.push(0);
			else
				camInitPosArray.push(1);
		}
		const camInitVector = new THREE.Vector3().fromArray(camInitPosArray);
		const camInitAhead = new THREE.Vector3().fromArray(camInitPosArray).multiplyScalar(-1);

		const xPosVector = new THREE.Vector3(1, 0, 0);
		const yPosVector = new THREE.Vector3(0, 1, 0);
		const zPosVector = new THREE.Vector3(0, 0, 1);
		const xNegVector = new THREE.Vector3(-1, 0, 0);
		const yNegVector = new THREE.Vector3(0, -1, 0);
		const zNegVector = new THREE.Vector3(0, 0, -1);
		...
		function prepareAnimationData(object, focusPoint) {
			...
			switch (object.userData.type) {
				...
				case 'posX':
					...
					targetQuaternion.setFromUnitVectors(camInitAhead, xNegVector);
					break;

				case 'posY':
					targetPosition.set(0, 1, 0);
					targetQuaternion.setFromUnitVectors(camInitAhead, yNegVector);
					break;

				case 'posZ':
					targetPosition.set(0, 0, 1);
					targetQuaternion.setFromUnitVectors(camInitAhead, zNegVector);
					break;

				case 'negX':
					targetPosition.set(- 1, 0, 0);
					targetQuaternion.setFromUnitVectors(camInitAhead, xPosVector);
					break;

				case 'negY':
					targetPosition.set(0, - 1, 0);
					targetQuaternion.setFromUnitVectors(camInitAhead, yPosVector);
					break;

				case 'negZ':
					targetPosition.set(0, 0, - 1);
					targetQuaternion.setFromUnitVectors(camInitAhead, zPosVector);
					break;
			...
			q1.setFromUnitVectors(camInitVector, editorCamera.position.normalize());

			// dummy.lookAt(targetPosition);
			// dummy.lookAt(camInitPosition);
			// console.log(new THREE.Vector3().setFromEuler(dummy.rotation).multiplyScalar(180 / Math.PI));
			// q2.copy(dummy.quaternion);
			q2.setFromUnitVectors(camInitVector, targetPosition);
			console.log(q2);
