	...
	function update( object ) {
		...
		if ( geometry && geometry.isBufferGeometry ) {
			...
			if ( index !== null ) {
				...
				containerAttributes.add( new UIText( String( index.count )).setFontSize( '12px' ) );
			...
			for ( const name in attributes ) {
				...
				containerAttributes.add( new UIText( String( attribute.count ) + ' (' + attribute.itemSize + ')' ).setFontSize( '12px' ) );
				containerAttributes.add( new UIBreak() );
