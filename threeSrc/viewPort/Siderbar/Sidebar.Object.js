...
function SidebarObject(editor) {
  ...
  const objectUUIDRenew = new UIButton(strings.getKey("sidebar/object/new"))
    ...
    .onClick(function () {
      ...
      editor.execute(
        new SetUuidCommand(
          editor,
          editor.selectionHelper.selectedObject[0],
          objectUUID.getValue()
        )
      );
    });
  ...
  function update() {
    const object = editor.selectionHelper.selectedObject[0];
  ...
  function getSelectedObj(objects) {
    let object = null;
    if (objects.length == 1) {
      object = objects[0];
    }
  ...
  signals.objectsChanged.add(function (objects) {
    if (objects[0] !== editor.selectionHelper.selectedObject[0]) return;

    updateUI(objects[0]);
  });

  signals.refreshSidebarObject3D.add(function (object) {
    if (object !== editor.selectionHelper.selectedObject[0]) return;
