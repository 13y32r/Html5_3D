 ...
 */
import { Vector2, EventDispatcher, Raycaster, BoxHelper, Layers } from "three";
...
class SelectionHelper extends EventDispatcher {
  constructor(selectionBox, editorOperate) {
    ...
    this.onPointerDown = function (event) {
      if (that.selectState != SelectState.IDLE) return;
  ...
  restart() {
    ...
    this.editorOperate.addEventListener(
      "editorPointerDown",
      this.onPointerDown
    );
    this.editorOperate.addEventListener(
  ...
  pause() {
    this.editorOperate.removeEventListener(
      "editorPointerDown",
      this.onPointerDown
    );
    this.editorOperate.removeEventListener(
  ...
  onSelectOver(event) {
    ...
    if (event.shiftKey) {
      for (let obj of selObject) {
        if (that.layers.test(obj.layers)) {
      ...
      for (let obj of selObject) {
        if (that.layers.test(obj.layers)) {
          that.selectedObject = that.selectedObject.filter(
            (item) => item != obj
          );
        }
      }
  ...
  onSingleOver(event) {
    ...
    let intersects = raycaster.intersectObjects(
      that.editorOperate.scene.children
    );
    ...
    if (intersects.length > 0) {
      ...
      for (let ele of intersects) {
        if (that.layers.test(ele.object.layers)) {
          if (event.shiftKey) {
            that.selectedObject.push(ele.object);
          } else if (event.altKey) {
            that.selectedObject = that.selectedObject.filter(
              (item) => item != ele.object
            );
          } else {
            that.selectedObject.length = 0;
  ...
  addObjectOutline() {
    ...
    if (that.outLineObject.length != 0) {
      for (let obj of that.outLineObject) {
        that.editorOperate.updateObjList =
          that.editorOperate.updateObjList.filter((item) => item != obj);
