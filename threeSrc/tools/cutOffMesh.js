...
class CutOffMesh extends Tool {
    ...
    cutOut() {
        ...
        const plane = new Plane().setFromCoplanarPoints(cutVector0, cutVector1, cutVector2);
        ...
        if (that.cutOBJ.length != 0) {
            for (let ele in that.cutOBJ) {
                ...
                mesh0.position.set(that.cutOBJ[ele].position.x, that.cutOBJ[ele].position.y, that.cutOBJ[ele].position.z);
                mesh1.position.set(that.cutOBJ[ele].position.x, that.cutOBJ[ele].position.y, that.cutOBJ[ele].position.z);
