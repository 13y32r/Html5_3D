...
class MenuGUI {
  ...
  creatGUI(menuObj, fatherBtn) {
    ...
    that[menuName].initX = document.body.clientWidth - 100;
    ...
    that[menuName].menu_main.setAttribute(
      "style",
      "top:" + that[menuName].initY + "px;left:" + that[menuName].initX + "px;"
    );
    ...
    for (let key in menuObj) {
      ...
      function cellInit() {
        ...
        if (menuObj[key].shortcut != undefined) {
          that[menuName][key].setAttribute(
            "title",
            menuObj[key].name + " (" + menuObj[key].shortcut[0] + ")"
          );
        } else {
          that[menuName][key].setAttribute("title", menuObj[key].name);
        }
        that[menuName][key].style.backgroundImage =
          "url(" + menuObj[key]["image_url"] + ")";

        if (menuObj[key].class != undefined && menuObj[key].class != "") {
          if (
            !that[menuName]["classArray"].includes(
              window[menuObj.Title + "_" + menuObj[key].class]
            )
          ) {
            window[menuObj.Title + "_" + menuObj[key].class] = new window[
              menuObj[key].class
            ]();
            that[menuName][key][menuObj[key].class] =
              window[menuObj.Title + "_" + menuObj[key].class];
            that[menuName]["classArray"].push(
              window[menuObj.Title + "_" + menuObj[key].class]
            );
          } else {
            that[menuName][key][menuObj[key].class] =
        ...
        that[menuName][key].btnDown = function () {
          if (menuObj[key].preEditorCondition != undefined) {
            if (
              window["editorOperate"].state !=
              EditorState[menuObj[key].preEditorCondition]
            ) {
              return;
            }
          }
          ...
          if (menuObj[key].btnDown != undefined) {
            if (
              menuObj[key].param != undefined &&
              Object.keys(menuObj[key].param).length != 0
            ) {
        ...
        function initParamWindow(paramObj) {
          ...
          initParam_body.setAttribute(
            "style",
            "top:" + initY + "px;left:" + initX + "px;"
          );
          ...
          initParam_drag.addEventListener("pointerdown", changeWidth);
          ...
          for (let key in paramObj) {
            ...
            initParam_cell_input.addEventListener("keydown", function (event) {
          ...
          setTimeout(function () {
            initParam_content.children[
              initParam_content.children.length - 1
            ].children[
              initParam_content.children[initParam_content.children.length - 1]
                .children.length - 1
            ].focus();
          }, 1);
          ...
          function changeWidth(event) {
            ...
            function dragStart() {
              ...
              document.body.addEventListener("pointercancel", dragEnd);
            }
            ...
            function dragMove(event) {
              ...
              initParam_main.style.width =
                initParam_main.offsetWidth - 2 - posE + "px";
            }
            ...
            function dragEnd() {
              ...
              document.body.removeEventListener("pointercancel", dragEnd);
            }
          }
        }
      }
      ...
      switch (menuObj[key].type) {
        case "folder":
          ...
          that[menuName][key].onmousedown = function () {
            ...
            if (!that[menuName][key].btnPress) {
              ...
              that[menuName][key].folderObj = that.creatGUI(
                that.listJson[key],
                that[menuName][key]
              );
        ...
        default:
          console.error(
            "从menu_list.json中加载了一个未知的“菜单”类型，请重新审核之后再运行。"
          );
      }
        ...
        that[menuName][key].shortcut = function (event) {
          ...
          if (menuObj[key].combkey != undefined) {
            if (menuObj[key].combkey[0] != undefined)
              if (!event[menuObj[key].combkey[0]]) return;

            if (menuObj[key].combkey[1] != undefined)
              if (!event[menuObj[key].combkey[1]]) return;
          } else {
            if (event.altKey || event.ctrlKey || event.shiftKey) return;
          }

          if (
            event.key == menuObj[key].shortcut[0] ||
            event.key == menuObj[key].shortcut[1]
          ) {
    ...
    function changeToVerticalStyle(target, name) {
      var v_name = name + "_V";
      var h_name = name + "_H";
      if (target.classList.contains(h_name)) target.classList.remove(h_name);
      if (!target.classList.contains(v_name)) target.classList.add(v_name);
    }

    function changeToHorizontalStyle(target, name) {
      var v_name = name + "_V";
      var h_name = name + "_H";
      if (target.classList.contains(v_name)) target.classList.remove(v_name);
      if (!target.classList.contains(h_name)) target.classList.add(h_name);
    }
